...
export class AuthService{
  ...
  createUser(name: string , contact: string , nic: string ,email: string ,password: string , role : string){
    ...
    this.http.post("http://localhost:5001/api/user/signup",authData)
      .subscribe(response =>{
  ...
  login(email: string, password){
    ...
    this.http.post<{token: string, expiresIn: number, role :string , message: string}>("http://localhost:5001/api/user/login",authData)
      .subscribe(response =>{
  ...
  getUser() {
    this.http.get<{message: string, users: any}>('http://localhost:5001/api/user/getUserData')
    .pipe(map(userData => {
  ...
  updateUser(id: string ,  name: string, email: string, nic: string, contact: string, password: string, role: string){
    ...
    this.http
             .put('http://localhost:5001/api/user/' +id , user)
             .subscribe(response => {
  ...
  deleteUser(userId: string) {
    this.http.delete('http://localhost:5001/api/user/' +userId)
      .subscribe(() => {
